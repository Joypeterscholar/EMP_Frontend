...
const EditIncident = ({ showModal, setShowModal, data, fetchData }) => {
  ...
  const handleSubmitIncidentForm = async (event) => {
    ...
    try {
      ...
      const response = await customFetch.put(
        // eslint-disable-next-line react/prop-types
        `/incident/update-incident/${data._id}`,
        formDataForIncident
      );
      ...
      {showModal && (
        <div className='fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 py-4'>
          <form
            ...
            <div className='mt-3'>
              <SubmitBtn text='Edit incident' isSubmitting={isSubmittingIncident} />
            </div>
          </form>
        </div>
      )}
